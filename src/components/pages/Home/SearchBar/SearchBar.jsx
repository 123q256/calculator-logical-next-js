...
const SearchBar = () => {
  ...
  const highlightMatch = (name) => {
    ...
    return name.replace(
      regex,
      (match) => `<strong class="text-black">${match}</strong>`
    );
  };
  ...
  return (
    <div ref={searchRef} className="max-w-[670px] mx-auto mt-2">
      <div className="w-full mx-auto px-5 lg:py-2 md:py-2 py-1">
        ...
        <div className="flex items-center relative rounded-full bg-white shadow-sm border border-gray-200 focus-within:ring-2 focus-within:ring-blue-500 transition-all duration-200">
          <input
            id="search-bars"
            name="search-bars"
            ref={inputRef}
            ...
            className="w-full rounded-full pl-5 pr-10 py-3 bg-gray-50 text-gray-800 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-0"
          />
        ...
        {suggestions.length > 0 && (
          <div className="relative">
            <ul
              id="suggestion"
              style={{ zIndex: 99 }}
              className="absolute autosearchcomplete-items space-y-1 max-h-80 overflow-y-auto text-start bg-white rounded-lg shadow-inner mt-[5px] w-full"
            >
              {suggestions.map((item, index) => (
                <Link
                  href={`/${item.link}`}
                  key={index}
                  className={`block items-center p-2 rounded hover:shadow-sm hover:bg-gray-50 ${
