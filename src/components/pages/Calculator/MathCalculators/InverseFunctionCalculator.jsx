                ...
                <div className="col-span-12">
                     <div className="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                        <div className="col-span-12 md:col-span-8 lg:col-span-8 flex items-center">
                          <label for="equ" className="label" id="heading2">{ data?.payload?.tech_lang_keys[1] } y = f(x) =</label>
                        </div>
                        <div className="col-span-12 md:col-span-4 lg:col-span-4 flex justify-end items-center">
                          <button type="button" className="flex items-center p-1" id="exampleLoadBtn" onClick={exampleLoadHandler}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-arrow-up-right size-4 me-1" ><path d="M7 7h10v10"></path><path d="M7 17 17 7"></path></svg>
                            Load Example
                        </button>
                        </div>
                    </div>
                        ...
                        <div className="w-full mt-3">
                          <div className="row">
                            <div className="col-12 text-[18px] md:text-[20px] overflow-auto">
                              ...
                              <p className="mt-3">
                                {data?.payload?.tech_lang_keys['3']} <InlineMath math={`y=${result?.tech_enter}`} />
                              </p>
                              <p className="mt-3">
                                {data?.payload?.tech_lang_keys['4']} <InlineMath math="x" />.
                              </p>
                              <p className="mt-3">
                                {data?.payload?.tech_lang_keys['5']}.
                              </p>
                              <p className="mt-3">
                                {data?.payload?.tech_lang_keys['6']}: <InlineMath math={`y=${result?.tech_enter}`} /> {data?.payload?.tech_lang_keys['7']} <InlineMath math={`x=${result?.tech_enter2}`} />
                              </p>
                              <p className="mt-3">
                                {data?.payload?.tech_lang_keys['8']}: <InlineMath math={`x=${result?.tech_enter2}`} /> {data?.payload?.tech_lang_keys['9']} <InlineMath math="y" />
                              </p>
                              <p className="mt-3">
