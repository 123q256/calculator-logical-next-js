...
// result
        ...
        if (val) {
          ...
          stepOutput.push(
            <p className="mt-3" key={i}>
              <InlineMath math={`f(${withVar}_${i}) = f(${limits[i]}) = ${result.tech_enter.replace(withVar, limits[i])} = ${val}`} />
            </p>
          );
        }
      });
    ...
    if (limits[i + 1] !== undefined) {
      ...
      if (val) {
        ...
        stepOutput.push(
          <p className="mt-3" key={i}>
            <InlineMath
              math={`f\\left(\\frac{${withVar}_${i}+${withVar}_${i + 1}}{2}\\right) = f\\left(\\frac{${limits[i]}+${limits[i + 1]}}{2}\\right) = f(${mid}) = ${result.tech_enter.replace(withVar, mid)} = ${val}`}
            />
          </p>
        );
      }
  ...
  return (
    <>
      ...
      <p className="mt-3">
        <InlineMath math={`${delta} \\left(${sumExpr}\\right) = ${result.tech_res}`} />
      </p>
    </>
  );
}
  ...
  steps.forEach((val, i) => {
    if (val && limits[i + 1] !== undefined) {
      ...
      stepOutput.push(
        <p className="mt-3" key={i}>
          <InlineMath
            math={`\\frac{f(${withVar}_${i}) + f(${withVar}_${i + 1})}{2} = \\frac{f(${limits[i]}) + f(${limits[i + 1]})}{2} = \\frac{${left} + ${right}}{2} = ${val}`}
          />
        </p>
      );
    }
                 ...
                 <label htmlFor="tech_type" className="label">
                    ...
                    <div className="mt-2">
                      <select
                        ...
                        onChange={handleChange}
                      >
                        <option value="1">{data?.payload?.tech_lang_keys["6"]} </option>
                        <option value="2">{data?.payload?.tech_lang_keys["7"]} </option>
                        <option value="3">{data?.payload?.tech_lang_keys["8"]} </option>
                 ...
                 <div className="rounded-lg flex items-center justify-center">
                    <div className="w-full mt-3">
                      <div className="w-full text-[16px] overflow-auto">
                        ...
                        <p className="mt-3">
                          {data?.payload?.tech_lang_keys['16']} <InlineMath math={`[${a}, ${b}]`} />{' '}
                          {data?.payload?.tech_lang_keys['25']} <InlineMath math={`n=${n}`} />{' '}
                          {data?.payload?.tech_lang_keys['17']} <InlineMath math={`\Delta ${withVar} = ${delta}`} />{' '}
