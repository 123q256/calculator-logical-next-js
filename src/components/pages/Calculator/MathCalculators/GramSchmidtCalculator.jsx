    ...
    >
        <form className="row" onSubmit={handleSubmit}>
          <div className="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg space-y-6 mb-3">
            ...
            <p className="col-span-12 text-[16px]"><strong>{data?.payload?.tech_lang_keys[3]}</strong></p>
            <div className="col-span-12 overflow-auto">
               <table className=" mt-3 " width={900}>
                  <tbody>
                    {Array.from({ length: parseInt(formData.tech_matrix2 || "1") }).map((_, rowIndex) => (
                      <tr key={rowIndex}>
                        {Array.from({ length: parseInt(formData.tech_matrix22 || "1") }).map((_, colIndex) => {
                          const key = `tech_matrix3${rowIndex + 1}_${colIndex + 1}`;
                          return (
                ...
                <div className="w-full mx-auto p-4 lg:p-8 md:p-8 result_calculator rounded-lg space-y-6 result">
                  ...
                  <div className="rounded-lg flex items-center justify-center">
                    <div className="w-full mt-3 text-[16px] overflow-auto">
                      <p className="mt-2 text-[18px] font-bold">{data?.payload?.tech_lang_keys[5]}</p>
                      ...
                      <BlockMath>
                        {result?.tech_all_vec
                          ?.map((vec, i) => {
                            return `V_${i + 1} = \\begin{bmatrix} ${renderMatrix(vec)} \\end{bmatrix}`;
                          })
                          .join(",\\quad ")}
                      </BlockMath>

                      <p className="mt-4 text-[18px] font-bold">Step by Step {data?.payload?.tech_lang_keys[8]}</p>
                      ...
                      <div className=" rounded-lg px-3 py-2 mt-2 ">
                        ...
                        {/* First step */}
                        <p className="mt-2 font-bold">{data?.payload?.tech_lang_keys[13]} 1</p>
                        <BlockMath>
                          {`
                        ...
                        {(result?.tech_pros_ans || []).map((proj, n) => (
                          <div key={n}>
                            <p className="mt-4 font-bold">{data?.payload?.tech_lang_keys[13]} {n + 2}</p>
                            ...
                            <BlockMath>
                              {`
                                \\text{proj}_{\\vec{u_1}}(\\vec{v_${n + 2}}) ${
                                  n !== 0 ? ` - \\text{proj}_{\\vec{u_2}}(\\vec{v_${n + 2}})` : ""
                              } = \\begin{bmatrix}
                                ${renderMatrix(proj)}
                              \\end{bmatrix}
                              `}
                            ...
                            <BlockMath>
                              {`
                                \\vec{e_${n + 2}} = \\frac{\\vec{u_${n + 2}}}{|\\vec{u_${n + 2}}|} = \\begin{bmatrix}
