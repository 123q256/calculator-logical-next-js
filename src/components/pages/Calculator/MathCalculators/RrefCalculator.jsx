        ...
        <form className="row" onSubmit={handleSubmit}>
          <div className="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg space-y-6 mb-3">
            ...
            <div className="lg:w-[80%] md:w-[90%] w-full mx-auto ">
                ...
                <p className="col-span-12 text-[16px] text-blue px-1"><strong>{data?.payload?.tech_lang_keys[1]}:</strong></p>
            ...
            <div className="col-span-3">
              ...
              />
                ...
                <p className="col-span-12 text-[16px] text-blue px-1"><strong>{data?.payload?.tech_lang_keys[3]}</strong></p>
                    ...
                    {Array.from({ length: matrix2 }, (_, rowIdx) => (
                      <tr key={rowIdx}>
                        {Array.from({ length: matrix22 }, (_, colIdx) => {
                          const inputName = `tech_matrix3${rowIdx + 1}_${colIdx + 1}`;
                          return (
                            <td key={colIdx}>
                    ...
                    <ResultActions lang={data?.payload?.tech_lang_keys} />
                      ...
                      <div className="rounded-lg flex items-center justify-center">
                        <div className="w-full mt-3 text-[16px] overflow-auto">
                          <p className="mt-2 text-[18px] font-bold">{data?.payload?.tech_lang_keys[4]}</p>
                          <BlockMath math={`\\begin{bmatrix} ${renderMatrix(result?.tech_matrix || [])} \\end{bmatrix}`} />

                          <p className="mt-2 font-bold">{data?.payload?.tech_lang_keys[5]}</p>
                          <p className="mt-2">{data?.payload?.tech_lang_keys[6]}:</p>
                        <BlockMath math={`\\begin{bmatrix} ${renderInputMatrix()} \\end{bmatrix}`} />

                          <p className="mt-2">{data?.payload?.tech_lang_keys[7]}:</p>
                        {Array.isArray(result?.tech_swap) &&
                          result.tech_swap.map((mat, idx) => (
                            <div key={idx} className="mt-2">
                              <p dangerouslySetInnerHTML={{ __html: result.tech_swap_line[idx] }} />
                              <BlockMath math={`\\begin{bmatrix} ${renderMatrix(mat)} \\end{bmatrix}`} />
                            </div>
                        ))}
