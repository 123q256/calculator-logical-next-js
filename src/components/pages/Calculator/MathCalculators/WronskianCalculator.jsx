...
for (let row = 0; row < input.length - 1; row++) {
  const rowItems = input.map((_, i) => {
    return `${fsArray[i]}(${formData?.tech_with})${derivative ? `^{${derivative}}` : ''}`;
...
const formattedMatrix = result?.tech_matrix?.replace(/\[/g, '|').replace(/\]/g, '|');
                   ...
                   <div className="rounded-lg flex items-center justify-center">
                      <div className="w-full mt-3">
                        <div className="w-full text-[16px] overflow-auto">
                          ...
                          <p className="mt-3">
                            <BlockMath
                              math={`W(${fs})(${formData?.tech_with}) = \\begin{vmatrix} ${wronskianSteps.join(' \\\\ ')} \\end{vmatrix}`}
                            />
                          </p>
                          ...
                          <p className="mt-3">
                            <BlockMath
                              math={`W(${fs})(${formData?.tech_with}) = \\begin{vmatrix} ${wronskianStepsWithInput.join(' \\\\ ')} \\end{vmatrix}`}
                            />
                          </p>
                          ...
                          <p className="mt-3">
                            <BlockMath math={`W(${fs})(${formData?.tech_with}) = ${formattedMatrix}`} />
                          </p>

                          <p className="mt-3">{data?.payload?.tech_lang_keys['12']}:</p>
                          <p className="mt-3">
                            <BlockMath math={`W(${fs})(${formData?.tech_with}) = ${formattedMatrix} = ${result?.tech_res}`} />
                          </p>
                        </div>
                      </div>
                    </div>
