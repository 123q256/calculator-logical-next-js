...
const handleSubmit = async (e) => {
  ...
  const matrixKeys = Object.keys(formData).filter((key) =>
    key.startsWith("tech_matrix") || key.startsWith("tech_matrix3")
  );
            ...
            {formError && (
             ...
             <div class="lg:w-[100%] md:w-[100%] w-full mx-auto ">
              <div class="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                <p class="col-span-12"><strong>{data?.payload?.tech_lang_keys['1']}{" A "}{data?.payload?.tech_lang_keys['2']}:</strong></p>
                  {/* Matrix A Row x Col */}
                  ...
                  <div className="col-span-2">
                    <select
                      ...
                      onChange={(e) => setFormData({ ...formData, tech_rows1: e.target.value })}
                    >
                      {[...Array(10)].map((_, i) => (
                  ...
                  <div className="col-span-2">
                    <select
                      ...
                      onChange={(e) => setFormData({ ...formData, tech_columns1: e.target.value })}
                    >
                      {[...Array(10)].map((_, i) => (
                        <option key={i + 1} value={i + 1}>{i + 1}</option>
                      ))}
                  ...
                  <p class="col-span-12"><strong>{data?.payload?.tech_lang_keys['1']}{" B "}{data?.payload?.tech_lang_keys['2']}:</strong></p>
                  {/* Matrix B Row x Col */}
                  <div className="col-span-2">
                    <select
                      name="tech_matrix2"
                      ...
                      onChange={(e) => setFormData({ ...formData, tech_matrix2: e.target.value })}
                    >
                      {[...Array(10)].map((_, i) => (
                        <option key={i + 1} value={i + 1}>{i + 1}</option>
                      ))}
                  ...
                  <div className="col-span-2">
                    <select
                      ...
                      onChange={(e) => setFormData({ ...formData, tech_matrix22: e.target.value })}
                    >
                      {[...Array(10)].map((_, i) => (
                        <option key={i + 1} value={i + 1}>{i + 1}</option>
                      ))}
                  ...
                  <div class="col-span-12">
                    <table className="w-full ">{renderMatrixTable("tech_matrix", "tech_rows1", "tech_columns1")}</table>
                  </div>
                  <p className="mt-4 col-span-12 font-bold">Matrix B</p>
                  <div class="col-span-12">
                    <table className="w-full ">{renderMatrixTable("tech_matrix3", "tech_matrix2", "tech_matrix22")}</table>
                  </div>
               </div>
              </div>
                ...
                <div className="w-full result mx-auto p-4 lg:p-8 md:p-8 bg-white rounded-lg shadow-md space-y-6 result">
                  <div>
                    ...
                    <div className="text-[16px] overflow-auto">
                      ...
                      <BlockMath math={getMatrixLatex("tech_matrix", rows1, cols1)} />

                      <p className="mt-2 text-[18px] font-bold">Matrix B:</p>
                      <BlockMath math={getMatrixLatex("tech_matrix3", matrix2, matrix22)} />

                      <p className="mt-2 text-[18px] font-bold">Multiplication Steps:</p>
