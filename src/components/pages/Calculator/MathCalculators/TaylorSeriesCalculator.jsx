...
if (formData?.tech_point === 0) {
  ...
  seriesStr = `\\frac{${result?.tech_eexe}}{0!}(${formData?.tech_with} - (${formData?.tech_point}))^{0}`;
    ...
    derivSteps.forEach((val, i) => {
      ...
      if (formData?.tech_point === 0) {
        seriesStr += ` + \\frac{${evaluated}}{${i + 1}!}${formData?.tech_with}^{${i + 1}}`;
      } else {
        seriesStr += ` + \\frac{${evaluated}}{${i + 1}!}(${formData?.tech_with} - (${formData?.tech_point}))^{${i + 1}}`;
      }
      ...
      <React.Fragment key={i}>
        <p className="mt-3 text-[18px]">
          {`${i + 1}. ${data?.payload?.tech_lang_keys['16']} ${i + 1} ${data?.payload?.tech_lang_keys['17']}:`}
        </p>
        <BlockMath
          math={`f^{(${i + 1})}(${formData?.tech_with}) = \\left(f^{(${i})}(${formData?.tech_with})\\right)^{'} = \\left(${per}\\right)^{'} = ${derivative}`}
        />
        
        <p className="mt-3">
          {`${data?.payload?.tech_lang_keys['18']} ${i + 1} ${data?.payload?.tech_lang_keys['19']}:`}
        </p>
        <BlockMath math={`\\left(f(${formData?.tech_point})\\right)^{${der}} = ${evaluated}`} />
                    ...
                    <div className="rounded-lg flex items-center justify-center">
                          <div className="w-full mt-3">
                            <div className="w-full text-[16px]">
                              <p className="bg-gray bordered rounded-lg px-3 py-2 mt-2 text-[18px] overflow-auto">
                                <BlockMath
                              ...
                              {result?.tech_efun && (
                                <div className="bg-gray bordered rounded-lg px-3 py-2 mt-2 overflow-auto">
                                  ...
                                  />
                                  <BlockMath
                                    math={`E = ${result?.tech_efun} - (${result?.tech_eser}) \approx ${Math.abs(result?.tech_err)}`}
                                  />
                                </div>
                              )}
                              ...
                              <div className="bg-gray bordered rounded-lg px-3 py-2 mt-2 overflow-auto">
                                <p className="mt-3">
                                  {data?.payload?.tech_lang_keys['8']} <InlineMath math={result?.tech_enter} /> {data?.payload?.tech_lang_keys['9']} <InlineMath math={`a = ${formData?.tech_point}`} /> {data?.payload?.tech_lang_keys['10']} <InlineMath math={`n = ${formData?.tech_n}`} />
                                </p>
                               <p className="mt-3">{data?.payload?.tech_lang_keys['12']}</p>
                              <BlockMath math={`f(${formData?.tech_with}) = \\sum\\limits_{k=0}^{\\infty} \\frac{f^{(k)}(a)}{k!}(${formData?.tech_with} - a)^k`} />

                              <p className="mt-3">{data?.payload?.tech_lang_keys['13']}</p>
                              <BlockMath math={`P(${formData?.tech_with}) = \\sum\\limits_{k=0}^{${formData?.tech_n}} \\frac{f^{(k)}(a)}{k!}(${formData?.tech_with} - a)^k`} />

                              <BlockMath math={`f^{(0)}(${formData?.tech_with}) = f(${formData?.tech_with}) = ${result?.tech_enter}`} />

                              <p className="mt-3">
                                {data?.payload?.tech_lang_keys['15']}:
                              </p>
                              <BlockMath math={`f(${formData?.tech_point}) = ${result?.tech_eexe}`} />
                              ...
                              <BlockMath math={`f(${formData?.tech_with}) \\approx P(${formData?.tech_with}) = ${result?.tech_series}`} />

                                {formData?.tech_find && !isNaN(Number(formData?.tech_find)) && (
                                  <>
                                    <p className="mt-3">
                                      {data?.payload?.tech_lang_keys['22']}: <BlockMath math={`f(${formData?.tech_find}) = ${result?.tech_efun}`} />
                                    </p>
                                    <p className="mt-3">
                                      {data?.payload?.tech_lang_keys['23']}: <BlockMath math={`P(${formData?.tech_find}) = ${result?.tech_eser}`} />
                                    </p>
                                    <p className="mt-3">
                                      {data?.payload?.tech_lang_keys['24']}: <BlockMath math={`E = |f(${formData?.tech_find}) - P(${formData?.tech_find})| = ${result?.tech_efun} - (${result?.tech_eser})`} />
                                    </p>
                                  </>
                                )}
                              </div>
