            ...
            <div className="lg:w-[60%] md:w-[80%] w-full mx-auto ">
              <div className="grid grid-cols-12 mt-3 gap-2 md:gap-4 lg:gap-4">
                <div className="col-span-12 flex items-center justify-center font-s-16 mb-2">
                    <div>&nbsp;</div>
                    <div className="px-2 w-full text-center"><strong>{data?.payload?.tech_lang_keys['5']}</strong></div>
                    <div className="px-2 w-full text-center"><strong>{data?.payload?.tech_lang_keys['6']}</strong></div>
                </div>
                ...
                {formData.tech_weight.map((_, i) => (
                  <div key={i} className="flex items-center justify-center mt-3">
                    ...
                    {formData.tech_weight.length > 1 && (
                      <img src="/images/delete_btn.png" alt="delete item" className="h-4 w-4 cursor-pointer " onClick={() => deleteRow(i)} />
                     ...
                     <div className="rounded-lg flex items-center justify-center">
                      <div className="w-full mt-3">
                        <div className="w-full">
                              ...
                              <tbody>
                                <tr>
                                  ...
                                  <td className="py-2 border-b">
                                    {Number(result?.tech_weighted_average).toFixed(2)}
                                  </td>
                                </tr>
                                <tr>
                                  <td className="py-2 border-b">
                         ...
                         <div className="mt-2">
                          <BlockMath
                            math={"\\bar{x} = \\frac{(x_1 \\times w_1) + (x_2 \\times w_2) + \\cdots + (x_n \\times w_n)}{w_1 + w_2 + \\cdots + w_n}"}
                        />
                        </div>

                            <div className="mt-2">
                              <BlockMath
                                math={`\\bar{x} = \\frac{${weightedTerms.join(' + ')}}{${weightTerms.join(' + ')}}`}
                              />
                            </div>

                            <div className="mt-2">
                              <BlockMath
                                math={`\\bar{x} = \\frac{${multipliedValues.join(' + ')}}{${result?.tech_weight_sum}}`}
                              />
                            </div>
                            ...
                            <div className="mt-2">
                              <BlockMath
                                math={`\\bar{x} = ${(suming / result?.tech_weight_sum).toFixed(2)}`}
                              />
                            </div>
                          </div>
                        </div>
