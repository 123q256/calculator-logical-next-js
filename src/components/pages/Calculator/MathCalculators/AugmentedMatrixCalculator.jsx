...
const renderMatrix = (matrix, matrix22) => {
  ...
  return (
    <BlockMath>
      {`\\left[\\begin{array}{${"c".repeat(matrix22)}} ${rows} \\end{array}\\right]`}
    </BlockMath>
  );
};
            ...
            <div className="lg:w-[100%] md:w-[100%] w-full mx-auto">
                ...
                <div className="col-span-12 overflow-auto">
                  <table id="matrix2" width={750}>
                    <tbody>
                      {Array.from({ length: Number(formData.tech_matrix2) }).map((_, rowIndex) => (
                        <tr key={rowIndex}>
                          {Array.from({ length: Number(formData.tech_matrix22) }).map((_, colIndex) => {
                            const name = `tech_matrix3${rowIndex + 1}_${colIndex + 1}`;
                            return (
                              <td key={colIndex}>
                  ...
                  <div>
                    ...
                    <div className="w-full overflow-auto text-[16px]">
                      <p className="mt-2 text-[18px] font-bold">
                        <InlineMath math={`\\text{${data?.payload?.tech_lang_keys[5] || ""}}`} />
                      </p>

                      {result?.tech_pz && renderMatrix(result.tech_pz, matrix22)}
