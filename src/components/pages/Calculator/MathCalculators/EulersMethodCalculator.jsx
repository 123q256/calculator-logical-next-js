            ...
            {formError && (
              ...
              <div className="lg:w-[60%] md:w-[80%] w-full mx-auto ">
                          ...
                          <div className="col-span-12">
                              ...
                              <label for="tech_EnterEq" className="label mt-4">{data?.payload?.tech_lang_keys['1']} y′=f(x,y)` or `y′=f(t,y)=:</label>
                                <button type="button" className="mt-3 flex border rounded-lg p-1  items-center" id="exampleLoadBtn"  onClick={exampleLoadHandler}>
                             ...
                             <label htmlFor="tech_with" className="label">
                                ...
                                <div className="mt-2">
                                  <select
                                    ...
                                    onChange={handleChange}
                                  >
                                    <option value="h">{data?.payload?.tech_lang_keys['3']} </option>
                                    <option value="n">{data?.payload?.tech_lang_keys['4']} </option>
                                  </select>
                                </div>
                          </div>
                          ...
                          <div className="col-span-6">
                            {formData.tech_with == "h" ? (
                              <>
                              <label htmlFor="tech_size" className="label"> {data?.payload?.tech_lang_keys["18"]} (h): </label>
                              </>
                            ):(
                              <>
                              <label htmlFor="tech_size" className="label"> {data?.payload?.tech_lang_keys["18"]} (n): </label>
                              </>
                            )}
                          ...
                          <div className="col-span-6">
                               <label htmlFor="tech_con" className="label">
                              {data?.payload?.tech_lang_keys["6"]}  <sub className="text-blue">1</sub>
                                </label>
                                <div className=" relative">
                                <input
                                  type="number"
                      ...
                      <div className="w-full mt-3">
                        <div className="w-full">
                          <div className="w-full text-center text-[20px]">
                            <p className="my-3">
                              <strong className="bg-[#2845F5] px-3 py-2 text-[23px] rounded-lg text-white">
                                <InlineMath math={`y_{${result?.tech_find}} = ${parseFloat(result?.tech_ans).toFixed(2)}`} />
                              </strong>
                            </p>
                          </div>
                          ...
                          <div className="w-full text-[16px] rounded-lg px-4 py-2 overflow-auto">
                            <p className="mt-3">
                              {data?.payload?.tech_lang_keys['9']} <InlineMath math={`y(${result?.tech_find})`} /> {data?.payload?.tech_lang_keys['10']}{' '}
                            ...
                            <p className="mt-3">
                              {data?.payload?.tech_lang_keys['14']} <InlineMath math={`y_{n+1} = y_n + h \\cdot f(${result?.tech_one}_n, y_n)`} />
                            </p>
                            ...
                            <ul className="px-3">
                              ...
                              <li className="py-2">
                                <InlineMath math={`${result?.tech_one}_0 = ${result?.tech_ini}`} />
                              </li>
                              <li className="py-2">
                                <InlineMath math={`y_0 = ${result?.tech_con}`} />
                              </li>
                              <li className="py-2">
                                <InlineMath math={`f(${result?.tech_one}, y) = ${result?.tech_enter}`} />
                              </li>
                            </ul>
                            ...
                            {tableRows.map((row) => (
                              <div key={row.i}>
                                <p className="mt-3">
                                  <strong>{data?.payload?.tech_lang_keys['17']} {row.i}</strong>
                                </p>
                                <p className="mt-3">
                                  <InlineMath math={`${result?.tech_one}_${row.i} = ${row.xPrev} + h = ${(parseFloat(row.xPrev) + parseFloat(h)).toFixed(2)}`} />
                                </p>
                                <p className="mt-3">
                                  <InlineMath math={`y(${result?.tech_one}_${row.i}) = y(${(parseFloat(row.xPrev) + parseFloat(h)).toFixed(2)}) = y_${row.i} = y_{${row.i - 1}} + h \\cdot f(${row.xPrev}, ${row.yPrev})`} />
                                </p>
                                <p className="mt-3">
                                  <InlineMath math={`= ${row.yPrev} + ${h} \\cdot ${row.slope} = ${row.yNext}`} />
                                </p>
                              </div>
                            ))}
                            ...
                            <p className="mt-3">
                              <strong>
                                {data?.payload?.tech_lang_keys['7']}: <InlineMath math={`y(${result?.tech_find}) = ${parseFloat(result?.tech_ans).toFixed(2)}`} />
                              </strong>
                            </p>

                            <div className="w-full mt-3 overflow-auto">
                              <table className="w-full text-center border-collapse border border-black">
                                <thead style={{ backgroundImage: 'linear-gradient(90deg,rgb(97, 114, 219), #2845F5)' }}>
                                  <tr className="text-white">
                                    <td><strong>{data?.payload?.tech_lang_keys['17']}</strong></td>
                                    <td><strong>x<sub>0</sub></strong></td>
                                    <td><strong>y<sub>0</sub></strong></td>
                                    <td><strong>slope</strong></td>
                                    <td><strong>y<sub>n</sub></strong></td>
                                  </tr>
                                </thead>
                                <tbody>
                                  {tableRows.map((row, idx) => (
                                    <tr key={idx}>
                                      <td className="border p-2">{row.i}</td>
                                      <td className="border p-2">{row.xPrev}</td>
                                      <td className="border p-2">{row.yPrev}</td>
                                      <td className="border p-2">{row.slope}</td>
                                      <td className="border p-2">{row.yNext}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
