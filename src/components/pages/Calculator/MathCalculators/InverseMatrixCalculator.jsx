    ...
    return result?.tech_swap_line?.map((line, i) => {
      ...
      return (
        <div key={i} className="mt-3">
          <p>{line}</p>
          <BlockMath math={`\\left[\\begin{array}{${"c".repeat(columnCount)}|${"c".repeat(columnCount)}}${matrixStr} \\end{array}\\right]`} />
        </div>
      );
    });
  };
              ...
              <div className="grid grid-cols-12 mt-3 gap-2 md:gap-4 lg:gap-4">
                ...
                <div className="col-span-12">
                  <button
                    ...
                    className="px-3 py-2 mt-1 mx-1 bg-[#2845F5] text-white rounded-lg"
                  >
                ...
                <div className="col-span-12">
                  ...
                  <div className="mt-2">
                    <select
                      ...
                      onChange={handleChange}
                    >
                      <option value="exp_col">{data?.payload?.tech_lang_keys["5"]}</option>
                      <option value="exp_row">{data?.payload?.tech_lang_keys["6"]}</option>
                    </select>
                  </div>
                </div>
              </div>
                ...
                <div className="w-full mx-auto p-4 lg:p-8 md:p-8 result_calculator rounded-lg space-y-6 result">
                  ...
                  <div className="rounded-lg flex items-center justify-center">
                      <div className="w-full mt-3 overflow-auto">
                        {result?.tech_det === 0 ? (
                          <p className="mt-2 text-[18px] font-bold">{result?.tech_inverse}</p>
                        ) : (
                          <BlockMath math={`\\begin{bmatrix} ${renderMatrix(result?.tech_inverse)} \\end{bmatrix}`} />
                        )}
                        ...
                        {formData?.tech_dtrmn_opts_method === "exp_col" ? (
                          <>
                            <BlockMath
                              math={`\\text{Calculate } \\begin{bmatrix} ${renderMatrix(result?.tech_zain)} \\end{bmatrix}^{-1} \\text{${data?.payload?.tech_lang_keys[9]} Gauss-Jordan Elimination ${data?.payload?.tech_lang_keys[10]}}`}
                            />
                            <p className="mt-3">
                            ...
                            <p className="mt-3">
                              ...
                              <a href="https://calculator-logical.com/determinant-calculator" className="text-blue-900" target="_blank" rel="noopener noreferrer">
                                Determinant Calculator
                              </a>
                            </p>
                            <BlockMath
                              math={`D = \\begin{vmatrix} ${renderMatrixDet(result?.tech_zain)} \\end{vmatrix} = ${result?.tech_det}`}
                            />
                            {result?.tech_det === 0 ? (
                          ...
                          <>
                            <BlockMath
                              math={`\\text{Calculate } \\begin{bmatrix} ${renderMatrix(result?.tech_zain)} \\end{bmatrix}^{-1} \\text{${data?.payload?.tech_lang_keys[9]} adjugate method.}`}
                            />
                              ...
                              <>
                                ...
                                {result?.tech_c_down?.map((val, index) => (
                                  <p className="mt-3" key={index}>
                                    <BlockMath
                                      math={`C_{${val}} = (-1)^{${result?.tech_minus_pow[index]}} \\begin{vmatrix} ${renderMatrix(result?.tech_all_cofy[index])} \\end{vmatrix} = ${result?.tech_allcofy_det[index]}`}
                                    />
                                ...
                                <BlockMath math={`\\begin{bmatrix} ${renderMatrix(result?.tech_final_cofa)} \\end{bmatrix}`} />

                                <p className="mt-3">{data?.payload?.tech_lang_keys[17]}:</p>
                                <BlockMath math={`\\begin{bmatrix} ${renderMatrix(result?.tech_ans_tran)} \\end{bmatrix}`} />
                                ...
                                <BlockMath
                                  math={`\\begin{bmatrix} ${result?.tech_ans_tran
                                    .map((row) =>
                                      row.map((val) => `\\dfrac{${val}}{${result?.tech_det}}`).join(" & ")
                                    )
                                    .join(" \\\\ ")} \\end{bmatrix}`}
                                />
