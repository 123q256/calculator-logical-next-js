                      ...
                      <div className="w-full mt-3">
                        <div className="w-full text-[16px] overflow-auto">
                          ...
                          <p className="mt-3">
                            ...
                            {data?.payload?.tech_lang_keys['8']} <InlineMath math={`n=${result?.tech_n}`} /> {data?.payload?.tech_lang_keys['8']}.
                          </p>

                          <p className="mt-3">
                            <InlineMath math={`\\int\\limits_{a}^{b} f(${result?.tech_with})\\, d${result?.tech_with} \\approx \\frac{\\Delta ${result?.tech_with} }{2}(f(${result?.tech_with}_0) + 2f(${result?.tech_with}_1) + 2f(${result?.tech_with}_2) + ... + f(${result?.tech_with}_n))`} />
                          </p>

                          <p className="mt-3">
                            {data?.payload?.tech_lang_keys['9']} <InlineMath math={`\\Delta ${result?.tech_with} = \\frac{b-a}{n}`} />
                          </p>

                          <p className="mt-3">
                            {data?.payload?.tech_lang_keys['10']} <InlineMath math={`a = ${result?.tech_lb},\\ b = ${result?.tech_ub},\\ n = ${result?.tech_n}`} />
                          </p>

                          <p className="mt-3">
                            {data?.payload?.tech_lang_keys['11']},{' '}
                            <InlineMath math={`\\Delta ${result?.tech_with} = \\frac{${result?.tech_ub}-${result?.tech_lb}}{${result?.tech_n}} = ${result?.tech_diff}`} />
                          </p>

                          <p className="mt-3">
                            {data?.payload?.tech_lang_keys['12']} <InlineMath math={`[${result?.tech_lb}, ${result?.tech_ub}]`} />{' '}
                            {data?.payload?.tech_lang_keys['13']} <InlineMath math={`n = ${result?.tech_n}`} />{' '}
                            {data?.payload?.tech_lang_keys['14']} <InlineMath math={`\\Delta ${result?.tech_with} = ${result?.tech_diff}`} />{' '}
                          ...
                          <p className="mt-3">
                            <InlineMath
                              math={`a = ${limitPoints.slice(0, -1).join(', ')}${limitPoints.length > 1 ? ', ' : ''}${limitPoints[limitPoints.length - 1]} = b`}
                            />
                          </p>
                          ...
                          {stepValues.map((value, i) => {
                            ...
                            const expression = result?.tech_enter?.replaceAll(result?.tech_with, `(${limit})`);
                            const latexExpr =
                          ...
                          <p className="mt-3">
                            {data?.payload?.tech_lang_keys['17']} <InlineMath math={`\\frac{\\Delta ${result?.tech_with}}{2} = ${result?.tech_diff / 2}`} />
                          </p>

                          <p className="mt-3">
                            <InlineMath math={`${result?.tech_diff / 2}(${showSum}) = ${result?.tech_res}`} />
                          </p>
                        </div>
                      </div>
                    </div>
