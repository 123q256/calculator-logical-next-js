          ...
          <div className="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg space-y-6 mb-3">
            ...
            <div className="lg:w-[100%] md:w-[100%] w-full mx-auto ">
                <div className="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                  <p className="col-span-12"><strong>{ data?.payload?.tech_lang_keys[1]}:</strong></p>
                    ...
                    <div className="w-full mt-3">
                      <div className="w-full">
                        <div className="w-full text-[16px] overflow-auto">
                          ...
                          <BlockMath math={`${result?.tech_rref}\\left[\\begin{matrix}${[...Array(Number(result?.tech_colum)).keys()].map(i => `x_${i+1}`).join('\\\\')}\\end{matrix}\\right] = \\left[\\begin{matrix}${[...Array(Number(result?.tech_row)).keys()].map(() => '0').join('\\\\')}\\end{matrix}\\right]`} />

                          {/* Null Space */}
                          <p className="mt-3">{data?.payload?.tech_lang_keys['7']}:</p>
                          {Number(result?.tech_total) === 0 ? (
                            <BlockMath math={`\\left[\\begin{matrix}${[...Array(Number(result?.tech_row)).keys()].map(() => '0').join('\\\\')}\\end{matrix}\\right]`} />
                          ) : (
                            <BlockMath math={result?.tech_null} />
                          )}

                          {/* Total Nullity */}
                          <p className="mt-3">{data?.payload?.tech_lang_keys['8']}: {result?.tech_total}</p>
                        </div>
                      </div>
                    </div>
                  </div>
