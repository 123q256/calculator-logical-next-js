...
useEffect(() => {
  ...
  if (equationRef.current) {
    const equation = `\\frac{${tech_dividend || "?"}}{${tech_divisor || "?"}}`;
...
const divby = !isNaN(parseFloat(result?.tech_divby)) ? parseFloat(result.tech_divby) : 0;
                ...
                <div className="grid grid-cols-12 mt-3   gap-2 md:gap-4 lg:gap-4">
                    <div className="col-span-12">
                        ...
                        <div className="relative">
                          ...
                          {/* Equation Preview */}
                          <div className="col-span-12">
                            <label htmlFor="equation_preview" className="font-s-14 text-blue">
                              Equation Preview
                            </label>
                            <p
                          ...
                          <div className={`col-12 text-[16px]`}>
                            <div className="bg-sky bordered rounded-lg px-3 py-2 mt-2 text-[16px] overflow-auto">
                              <p className="mt-2">
                                ...
                                <span dangerouslySetInnerHTML={renderMath(result?.tech_quot)} />
                              </p>
                              <p className="mt-3">
                                <strong>Remainder = </strong>
                                <span dangerouslySetInnerHTML={renderMath(result?.tech_rmnd)} />
                              </p>
                            </div>
                            ...
                            <div className="bg-sky bordered rounded-lg px-3 py-2 mt-2 overflow-auto synth_steps">
                              {/* Start of steps */}
                              <p className="mt-3" dangerouslySetInnerHTML={renderMath(`\\dfrac{${result?.tech_eq}}{${result?.tech_eq1}}`)} />
                              <p className="mt-3">{data?.payload?.tech_lang_keys[6]}</p>

                              <p className="mt-3" dangerouslySetInnerHTML={renderMath(`(${coeffs1.slice(0, -1).join(", ")})`)} />
                              <p className="mt-3">{data?.payload?.tech_lang_keys[7]}</p>

                              <p className="mt-3" dangerouslySetInnerHTML={renderMath(`${result?.tech_eq1} = 0`)} />
                              {result?.tech_divby1.includes("frac") && (
                                <p className="mt-3" dangerouslySetInnerHTML={renderMath(`x = ${result?.tech_divby1.replace("frac", "dfrac")}`)} />
                              )}
                              <p className="mt-3" dangerouslySetInnerHTML={renderMath(`x = ${result?.tech_divby}`)} />
                              <p className="mt-3">{data?.payload?.tech_lang_keys[8]}</p>

                              {/* Initial synthetic division box */}
                              <p className="mt-4" dangerouslySetInnerHTML={renderMath(`
                                \\begin{array}{c|${"r".repeat(coeffs.length - 1)}} & ${coeffs.slice(1).map((_, i) => `x^{${coeffs.length - 2 - i}}`).join("&")} \\\\
                                ${result?.tech_divby} & ${coeffs.slice(0, -1).join("&")} \\\\ \\hline & 
                              \\end{array}`)} />

                              <p className="mt-3">{data?.payload?.tech_lang_keys[9]}</p>

                              <p className="mt-4" dangerouslySetInnerHTML={renderMath(`
                                \\begin{array}{c|${"r".repeat(coeffs.length - 1)}}${result?.tech_divby}& ${coeffs.slice(0, -1).join("&")} \\\\ \\hline & ${coeffs[0]} \\end{array}
                              `)} />

                              {/* Dynamic Steps */}
                              {steps.map((step, index) => (
                                <React.Fragment key={index}>
                                  <p className="mt-3">{data?.payload?.tech_lang_keys[step.type === "multiply" ? 10 : 11]}</p>
                                  <p className="mt-3" dangerouslySetInnerHTML={renderMath(step.equation)} />
                                  <p className="mt-4" dangerouslySetInnerHTML={renderMath(`
                                    \\begin{array}{c|${"r".repeat(coeffs.length - 1)}}${result?.tech_divby}& ${coeffs.slice(0, -1).join("&")} \\\\
                                    & ${step.carry.join("&")} \\\\ \\hline & ${coeffs[0]} & ${step.muls.join("&")} \\end{array}
                                  `)} />
                                </React.Fragment>
                              ))}
                              ...
                              {/* Final Result */}
                              <p className="mt-4">
                                {data?.payload?.tech_lang_keys[12]} {result?.tech_quot}, {data?.payload?.tech_lang_keys[13]} {result?.tech_rmnd}
                              </p>
                              <p className="mt-3">{data?.payload?.tech_lang_keys[14]}:</p>

                              <p className="mt-4" dangerouslySetInnerHTML={renderMath(`
                                \\dfrac{${result?.tech_eq}}{${result?.tech_eq1}} = ${result?.tech_quot} ${parseFloat(result?.tech_rmnd) < 0 ? "-" : "+"} \\dfrac{${Math.abs(result?.tech_rmnd)}}{${result?.tech_eq1}} ${result?.tech_rmnd === "0" ? "= " + result?.tech_quot : ""}
                              `)} />
                            </div>
                          </div>
                        </div>
