                ...
                <div className="col-span-12 md:col-span-6 lg:col-span-6">
                    ...
                    <div className="mt-2">
                      <select
                        ...
                        onChange={handleChange}
                      >
                        <option value="all">{data?.payload?.tech_lang_keys["all"]} </option>
                ...
                <div className="col-span-12 md:col-span-6 lg:col-span-6">
                    ...
                    <div className="mt-2">
                      <select
                        ...
                        onChange={handleChange}
                      >
                        <option value="Maintain">{data?.payload?.tech_lang_keys["main"]} </option>
                        <option value="Fat Loss">{data?.payload?.tech_lang_keys["loss_20"]} </option>
                        <option value="Loss 10%">{data?.payload?.tech_lang_keys["loss_10"]} </option>
                        <option value="Muscle Gain">{data?.payload?.tech_lang_keys["gain"]} </option>
                      </select>
                    </div>
                </div>
                <div className="col-span-12 md:col-span-6 lg:col-span-6">
                   <label htmlFor="tech_activity" className="label">
                    ...
                    <div className="mt-2">
                      <select
                        ...
                        onChange={handleChange}
                      >
                        <option value="Sedentary">{data?.payload?.tech_lang_keys["a1"]} </option>
                        <option value="Lightly Active">{data?.payload?.tech_lang_keys["a2"]} </option>
                        <option value="Moderately Active">{data?.payload?.tech_lang_keys["a3"]} </option>
                        <option value="Very Active">{data?.payload?.tech_lang_keys["a4"]} </option>
                        <option value="Extremely Active">{data?.payload?.tech_lang_keys["a5"]} </option>
                      </select>
                    </div>
                </div>
                <div className="col-span-12 md:col-span-6 lg:col-span-6">
                    ...
                    <div className="mt-2">
                      <select
                        ...
                        onChange={handleChange}
                      >
                        <option value="2nd">{data?.payload?.tech_lang_keys["9"]} </option>
                        <option value="first">{data?.payload?.tech_lang_keys["10"]} </option>
                        <option value="3rd">{data?.payload?.tech_lang_keys["11"]} </option>
                      </select>
                    </div>
                </div>
                    ...
                    <ResultActions lang={data?.payload?.tech_lang_keys} />
                      ...
                      <div className="rounded-lg  flex items-center justify-center">
                          <div className="w-full bg-light-blue result p-3 rounded-lg mt-3">
                              ...
                              <div className="w-full">
                                ...
                                <div className="flex flex-wrap items-center bg-blue-100 border border-blue-500 text-center rounded-lg px-1 tabs">
                                  {[
                                    ...
                                    >
                                      <div className={`bg-white px-3 py-2 cursor-pointer rounded-md transition-colors duration-300 hover_tags hover:text-white ${activeTab === tab.key ? 'bg-blue-200' : ''}`}>
                                  ...
                                  <div className="w-full">
                                    <div className="grid grid-cols-12 gap-2 mt-3">
                                      {['protein', 'fat', 'carbs'].map((key) => (
                                        <div key={key} className="col-span-12 md:col-span-4 lg:col-span-4">
                                          <fieldset className="rounded-[10px]">
                                            <legend className="text-blue-700 px-1">
                                              {data?.payload?.tech_lang_keys[key.toUpperCase()]}
                                            </legend>
                                            <input
                                              type="range"
                                              min={key === 'protein' ? 10 : key === 'fat' ? 20 : 45}
                                              max={key === 'protein' ? 35 : key === 'fat' ? 35 : 65}
                                              value={ownValues[key]}
                              ...
                              {/* Result UI */}
                                  <p className="w-full mt-3 mb-1">{data?.payload?.tech_lang_keys['before_res']}</p>
                                    <div className="grid grid-cols-12 gap-2 md:gap-4 lg:gap-4">
                                      <div className="col-span-12 md:col-span-9 lg:col-span-9">
                                        <div className="flex flex-wrap items-center justify-between relative bg-sky border rounded px-3 pb-2 mt-3">
                                          <div className="flex items-center px-3 mt-2">
                                            <img src="/images/calories1.png" alt="Calories" width="60" height="60" />
                                            <strong className="text-blue text-lg ms-2">{data?.payload?.tech_lang_keys['CALORIES']}</strong>
                                          </div>
                                          <div className="px-3 mt-2">
                                            <div className="text-[32px] text-[#908310] font-bold">{result?.tech_calories || 0}</div>
                                            <div>{formData?.tech_meal === 'all' ? data?.payload?.tech_lang_keys['C_per_day'] : data?.payload?.tech_lang_keys['cpm']}</div>
                                          </div>
                                          <div className="overflow-auto px-3 mt-2">
                                            <table className="w-full text-sm">
                                              <tbody>
                                                <tr>
                                                  <td className="border-b py-2 text-[#166fa5]">{data?.payload?.tech_lang_keys['PROTEIN']}</td>
                                                  <td className="border-b ps-5">{currentValues.protein}%</td>
                                                </tr>
                                                <tr>
                                                  <td className="border-b py-2 text-[#299fce]">{data?.payload?.tech_lang_keys['FAT']}</td>
                                                  <td className="border-b ps-5">{currentValues.fat}%</td>
                                                </tr>
                                                <tr>
                                                  <td className="py-2 text-[#64d3ff]">{data?.payload?.tech_lang_keys['CARBS']}</td>
                                                  <td className="ps-5">{currentValues.carbs}%</td>
                                                </tr>
                                              </tbody>
                                            </table>
                                          </div>
                                      ...
                                      <div className="col-span-12 md:col-span-3 lg:col-span-3">
                                        <div className="bg-white border rounded-full overflow-hidden">
                                  ...
                                  <p className="mt-2">{data?.payload?.tech_lang_keys['after_res']}</p>
                                  <div className="grid grid-cols-12 gap-2 md:gap-4 lg:gap-4">
                                    {displayData.map((item) => (
                                      <div key={item.label} className="col-span-12 md:col-span-6 lg:col-span-6">
                                        <div className="flex items-center justify-between bg-sky border rounded-lg p-3">
                                          ...
                                          <div>
                                            <div className="text-blue text-xl font-bold">{item.value}</div>
                                            <div className="text-xs">
                                              {formData?.tech_meal === 'all'
                                                ? data?.payload?.tech_lang_keys['grams_per_day']
                                                : data?.payload?.tech_lang_keys['gpm']}
                                            </div>
                                            {item.range && (
                                              <div className="text-xs text-gray">
                                                {data?.payload?.tech_lang_keys['range']} {item.range}
                                              </div>
                                            )}
                                          </div>
                                        </div>
