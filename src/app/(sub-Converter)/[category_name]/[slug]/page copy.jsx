...
const SubConverter = ({ unit1 = "", unit2 = "" }) => {
  ...
  console.log('Category Name:', categoryName, 'Slug:', slug);
  ...
  const handleFetchDetails = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load converter data");
    }
  ...
  const formatLinkTitle = (link) => {
    ...
    const relevantPart = cal_title_parts.length > 1 ? cal_title_parts[1] : cal_title_parts[0];
  ...
  try {
    ...
    console.error("Error parsing tech_lang_keys:", error, "String was:", langKeysString);
  ...
  const results = (final_res) => {
    if (final_res === "Error" || isNaN(final_res) || !isFinite(final_res)) return "Error";
  ...
  const metaTitle = data?.payload?.calculator?.tech_meta_title || "Unit Converter";
  const metaDescription = data?.payload?.calculator?.tech_meta_des || "Convert between different units";
      ...
      <ConverterWrap
        ...
        links={[
          { name: "Home", path: "/" },
          {
            ...
            path: "/" + (data?.payload?.calculator?.tech_cal_cat || "converters"),
          },
          {
            name: data?.payload?.calculator?.tech_calculator_title || "Unit Converter",
              ...
              <div className="col-span-2 ">
                <div className="grid grid-cols-12 gap-2 bg-white shadow-md my-2 p-2 content">
                  <p className="p-2 text-[18px] mt-5 col-span-12">
                    <strong>
                      {langObject["1"] || "Volume"} {langObject["2"] || "Converters"}
                    </strong>
                  </p>
